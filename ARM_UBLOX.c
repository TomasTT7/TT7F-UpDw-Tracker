...
uint16_t CRC16_checksum(uint8_t *string, uint8_t len)
{
	uint8_t i;
	uint16_t crc;
	uint8_t c;
...
void ASCII_8bit_transmit(uint8_t number, uint8_t *buffer)
{
...
void ASCII_8bit_2DEC_transmit(uint8_t number, uint8_t *buffer)
{
	uint8_t ascii1 = 0; // 8bit numbers - 3 decimal places max
	uint8_t ascii2 = 0;
	uint8_t ascii3 = 0;
...
void ASCII_16bit_transmit(uint16_t number, uint8_t *buffer)
{
...
void ASCII_16bit_HEX_transmit(uint16_t number, uint8_t *buffer)
{
...
void ASCII_32bit_transmit(uint32_t number, uint8_t *buffer)
{
...
void ASCII_32bit_LATLON_transmit(uint32_t number, uint8_t *buffer)
{
	uint8_t ascii1 = 0; // 32bit numbers - 10 decimal places max
	uint8_t ascii2 = 0;
	uint8_t ascii3 = 0;
...
void TX_BUFFER_fill_UBX(uint8_t *buffer)
{
	buffer[0] = '$';
	buffer[1] = '$';
	buffer[2] = 'T';
	...
	buffer[6] = ',';
	sequence = 7;
	ASCII_32bit_transmit(telemCount, buffer);
	buffer[sequence++] = ',';
	ASCII_8bit_2DEC_transmit(GPShour, buffer);
	buffer[sequence++] = ':';
	ASCII_8bit_2DEC_transmit(GPSminute, buffer);
	buffer[sequence++] = ':';
	ASCII_8bit_2DEC_transmit(GPSsecond, buffer);
	buffer[sequence++] = ',';
	if(GPSfix > 0) // if the fix is not valid, retransmit last valid position data
	{
		if(GPSlat < 0) buffer[sequence++] = '-';
		ASCII_16bit_transmit((uint16_t)GPSlat_int, buffer);
		buffer[sequence++] = '.';
		ASCII_32bit_LATLON_transmit((uint32_t)GPSlat_dec, buffer); // must fill '0's after decimal point
		buffer[sequence++] = ',';
		if(GPSlon < 0) buffer[sequence++] = '-';
		ASCII_16bit_transmit((uint16_t)GPSlon_int, buffer);
		buffer[sequence++] = '.';
		ASCII_32bit_LATLON_transmit((uint32_t)GPSlon_dec, buffer); // must fill '0's after decimal point
		buffer[sequence++] = ',';
		if(GPSalt < 0) buffer[sequence++] = '-';
		uint32_t alt = (uint32_t)abs(GPSalt);
		ASCII_32bit_transmit(alt, buffer);
		GPSlat_L = GPSlat;
		...
		ASCII_16bit_transmit((uint16_t)GPSlat_int_L, buffer);
		buffer[sequence++] = '.';
		ASCII_32bit_LATLON_transmit((uint32_t)GPSlat_dec_L, buffer); // must fill '0's after decimal point
		buffer[sequence++] = ',';
		if(GPSlon_L < 0) buffer[sequence++] = '-';
		ASCII_16bit_transmit((uint16_t)GPSlon_int_L, buffer);
		buffer[sequence++] = '.';
		ASCII_32bit_LATLON_transmit((uint32_t)GPSlon_dec_L, buffer); // must fill '0's after decimal point
		buffer[sequence++] = ',';
		...
		ASCII_32bit_transmit(alt_L, buffer);
	}
	buffer[sequence++] = ',';
	ASCII_8bit_transmit(GPSsats, buffer);
	buffer[sequence++] = ',';
	buffer[sequence++] = GPSfix + '0';
	//TXbuffer[sequence++] = ',';
	//ASCII_16bit_transmit(AD3data, TXbuffer);
	//TXbuffer[sequence++] = ',';
	//ASCII_16bit_transmit(AD9data, TXbuffer);
	...
	GPSchecksum = CRC16_checksum(buffer, sequence);
	buffer[sequence++] = '*';
	ASCII_16bit_HEX_transmit(GPSchecksum, buffer);
	buffer[sequence++] = 0x0D; // CR '\r'
...
void TX_BUFFER_fill_NMEA(uint8_t *buffer)
{
	buffer[0] = '$';
	buffer[1] = '$';
	buffer[2] = 'T';
	...
	if(GPSfix > 0 && GPSchecksum == GPSchecksum_verify) // if the fix is not valid or the GGA message was corrupt, retransmit last valid position data
	{
		...
		ASCII_32bit_transmit(GPSalt, buffer);
		GPSlat_int_L = GPSlat_int;
		GPSlat_dec_L = GPSlat_dec;
		GPSlon_int_L = GPSlon_int;
		GPSlon_dec_L = GPSlon_dec;
		...
		ASCII_32bit_transmit(GPSalt_L, buffer);
	}
	buffer[sequence++] = ',';
	ASCII_8bit_transmit(GPSsats, buffer);
	buffer[sequence++] = ',';
	ASCII_8bit_transmit(GPSfix, buffer);
	buffer[sequence++] = ',';
	...
	//ASCII_16bit_transmit((uint16_t)AD3, buffer);
	//buffer[sequence++] = ',';
	ASCII_16bit_transmit((uint16_t)AD9, buffer);
	buffer[sequence++] = ',';
	if(temperatureF < 0) buffer[sequence++] = '-';
	ASCII_8bit_transmit((uint8_t)abs(temperatureF), buffer);
	buffer[sequence++] = ',';
